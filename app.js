...
async function memberDash(){page('member');let r=await A.post('member',{token:A.token()});if(!r.ok)return login();let d=r.data,next=d.events.find(x=>new Date(x.DateTimeISO)>new Date());shell(`<section class="section"><div class="topActions"><button class="btn" onclick="home()">Home</button><button class="btn" onclick="logout()">Logout</button></div><h1 class="title">Welcome, ${A.esc(d.user.fullName)}</h1><p class="subtitle">Team: <b>${A.esc(d.user.team)}</b></p>${next?`<div class="card"><h3>Nearest Event</h3><div class="count" id="memberCount"></div><p>${A.esc(next.Title)} · ${A.esc(next.Location||'')}</p></div>`:''}<div class="grid2"><div class="card"><h2>Tasks</h2>${d.tasks.length?`<table class="table"><tr><th>Task</th><th>Deadline</th><th>Status</th></tr>${d.tasks.map(t=>`<tr><td>${A.esc(t.Title)}<br><span class="mini">${A.esc(t.Description)}</span></td><td>${A.esc(t.DeadlineISO)}</td><td>${A.esc(t.Status)}<br><button class="btn" onclick="submitTaskUI('${t.TaskID}')">Submit</button></td></tr>`).join('')}</table>`:'<p class="muted">No tasks yet.</p>'}</div><div class="card"><h2>Schedule</h2>${d.events.length?d.events.slice(0,10).map(x=>`<div class="status" style="margin:8px 0"><b>${A.esc(x.Title)}</b><br>${A.esc(x.DateTimeISO)}<br>${A.esc(x.Location||'')} ${x.MeetingLink?`<a href="${A.esc(x.MeetingLink)}" target="_blank">Join</a>`:''}</div>`).join(''):'<p class="muted">No events yet.</p>'}</div></div></section>`);if(next)countdown(new Date(next.DateTimeISO),1)}
function submitTaskUI(id){let link=prompt('Paste submission link (GitHub/Drive/etc.)');if(link===null)return;let comment=prompt('Comment (optional)')||'';A.post('submitTask',{token:A.token(),taskId:id,link,comment}).then(r=>{if(!r.ok)A.toast(r.error);else{A.toast('Task submitted');memberDash()}})}
async function adminDash(){page('admin');let r=await A.post('admin',{token:A.token()});if(!r.ok)return login();let d=r.data;window.ADMIN=d;shell(`<section class="section"><div class="topActions"><button class="btn" onclick="home()">Home</button><button class="btn" onclick="logout()">Logout</button></div><h1 class="title">Admin Dashboard</h1><p class="subtitle">${A.esc(d.role)} · Full management access according to permissions.</p><div class="grid"><div class="card"><h3>Applicants</h3><div class="count">${d.apps.length}</div></div><div class="card"><h3>Members</h3><div class="count">${d.members.length}</div></div><div class="card"><h3>Teams</h3><div class="count">${d.teams.length}</div></div></div><div class="card" style="margin-top:18px"><div class="adminTabs"><button class="btn primary" onclick="showApps()">Applicants</button><button class="btn" onclick="showMembers()">Members</button><button class="btn" onclick="showTeams()">Teams</button><button class="btn" onclick="showCreate()">Create Member</button><button class="btn" onclick="showTaskCreate()">Create Task</button><button class="btn" onclick="showEventCreate()">Create Event</button><button class="btn" onclick="showSlotCreate()">Interview Slots</button></div><div id="adminArea"></div></div></section>`);showApps()}
